...
public class ChessMatch {
	...
	private Piece makeMove(Position source, Position target) {
		...
		//Special Move castling little Rook
		...
		//Special Move castling BIG Rook
		if (p instanceof King && target.getColumn() == source.getColumn()-2) {
	...
	private void undoMove(Position source, Position target, Piece capturedPiece) {
		...
		//UNDOOO Special Move castling little Rook
		if (p instanceof King && target.getColumn() == source.getColumn()+2) {
			Position sourceT = new Position(source.getRow(),source.getColumn()+3);
			Position targetT = new Position(target.getRow(), target.getColumn()+1);
		...
		//UNDOOO Special Move castling BIG Rook
		if (p instanceof King && target.getColumn() == source.getColumn()-2) {
			Position sourceT = new Position(source.getRow(),source.getColumn()-4);
			Position targetT = new Position(target.getRow(), target.getColumn()-1);
			ChessPiece rook = (ChessPiece)board.removePiece(targetT);
	...
	private boolean testCheckMate(Color color) {
		...
		for (Piece p : list) {
			...
			for (int i = 0;i<board.getRows();i++) {
				for (int j=0;j<board.getColumns();j++) {
					if (mat[i][j]) {
						//set to position by downcasting
						Position source = ((ChessPiece)p).getChessPosition().toPosition();
						// set new Position by mat i j
	...
	private void initialSetup() {
        ...
        */
        ...
        //This auto reference for this case match
